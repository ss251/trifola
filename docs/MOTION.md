...
| M-2 `ContentColumn` | Section identity changes under a pointer transaction | Occasional | Incoming opacity + y 8→0 with `nav`; outgoing opacity with `exit` | 160ms cross-fade, no offset |

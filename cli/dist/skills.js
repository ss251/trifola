...
export function splitFrontmatter(text) {
    ...
    if (endIdx === -1)
        return { frontmatter: null, body: text }; // unterminated fence -> treat as body
...
function parseBlock(block) {
    ...
    while (i < block.length) {
        ...
        const line = raw.trim();
        i += 1;
        if (line.length === 0 || raw.startsWith(" ") || raw.startsWith("\t"))
            continue;
...
// MARK: - Catalog
function loadSkill(id, manifestPath, fileCount) {
    let text;
    try {
 ...
 */
export function scanUserSkills(directory) {
    let entries;
    try {

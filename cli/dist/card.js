...
export function renderCard(finding) {
    ...
    lines.push(`  ${teal(bold(`${fmtCount(finding.deadCount)} of ${fmtCount(finding.catalogCount)}`))} catalog skills never fired, ` +
        `across ${fmtCount(finding.claudeSessionCount)} Claude sessions${finding.subagentRunsByProvider.claude > 0
    ...
    if (finding.unsupportedPricingModeEntries > 0) {
        lines.push(`  ${fmtCount(finding.unsupportedPricingModeEntries)} entries used fast/batch pricing modes ` +
...
export function renderJSON(finding) {
    return {
        deadSkills: {
        ...
        freshInput: {
            ...
            note: "premiumUsd is the fresh-input premium above an all-cache-read floor; the avoidable share is unknowable from logs; firstTouchUsd is unavoidable cache-build cost — never sum the two",

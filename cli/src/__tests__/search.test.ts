...
describe("Claude conversation search", () => {
  ...
  test("empty corpus is honest in text and JSON modes", async () => {
    await withTempDir("trifola-search-empty-", async (root) => {
      ...
      assert.match(text.stdout, /No Claude Code sessions, Codex rollouts, or Grok sessions found/);
      assert.match(text.stdout, /Claude Code \+ Codex \+ Grok conversation text/);
